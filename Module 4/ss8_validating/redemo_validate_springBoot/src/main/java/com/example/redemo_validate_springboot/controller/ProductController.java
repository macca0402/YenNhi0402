package com.example.redemo_validate_springboot.controller;
...
public class ProductController {
    ...
    @GetMapping("/view")
    public String showView(@PathVariable(value = "id") int id, Model model) {
    ...
    public String showListPagingSlice(Model model,
                                      @RequestParam(name = "page", defaultValue = "1", required = false) int pageNumber,
                                      @RequestParam(name = "pageSize", required = false, defaultValue = "5") int pageSize) {
        Pageable pageable = PageRequest.of(pageNumber - 1, pageSize, Sort.by(Sort.Direction.DESC, "price"));
